...
class DockerRuntime(AbstractRuntime):
    ...
    def _create_container(self, scan_id: str, max_retries: int = 2) -> Container:
        ...
        if not image_name:
            raise ValueError("ESPRIT_IMAGE must be configured")
    ...
    def _get_or_create_container(self, scan_id: str) -> Container:
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"esprit-scan-id={scan_id}"}
            )
            if containers:
    ...
    # Directories to exclude when copying local source to container
    EXCLUDE_DIRS = {
        "node_modules",
        ".git",
        "__pycache__",
