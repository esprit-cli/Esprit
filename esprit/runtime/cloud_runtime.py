            ...
            while time.monotonic() < deadline:
                ...
                if response.status_code == 200:
                    ...
                    if status in {"stopped", "failed", "error", "terminated"}:
                        # Under ECS/API eventual consistency, this can flip
                        # transiently before task/tag discovery stabilizes.
                        # Keep polling until deadline.
                        await asyncio.sleep(_TOOL_SERVER_READY_POLL_INTERVAL_SECONDS)
                        continue
                elif response.status_code in {404, 410}:
                    # A newly-created sandbox can return 404/410 while the
                    # backend route/task mapping is still converging.
                    await asyncio.sleep(_TOOL_SERVER_READY_POLL_INTERVAL_SECONDS)
                    continue
                await asyncio.sleep(_TOOL_SERVER_READY_POLL_INTERVAL_SECONDS)
        return None
