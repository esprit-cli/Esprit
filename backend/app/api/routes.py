...
async def create_pr_for_scan(
    ...
    """
    Create a Pull Request for a completed repository scan from its stored patch artifact.
    """
    import structlog
    logger = structlog.get_logger()
    ...
    try:
        ...
        target_type = str(scan.get("target_type") or "repository")
        repo_full_name = pr_service.resolve_repo_full_name(str(scan.get("target") or ""), target_type)
        ...
        patch_s3_key = pr_service.resolve_patch_s3_key(scan, user.sub)
        patch_text = pr_service.download_patch_text(patch_s3_key)
        base_branch = str(scan.get("repo_branch") or "") or await pr_service.get_repo_default_branch(
            repo_full_name,
            github_token,
        )
        fix_branch = f"esprit/security-fixes-{scan_id[:8]}"
...
"""
        ...
        prepared_branch = pr_service.prepare_fix_branch(
        ...
        pull_request = await pr_service.create_pull_request(
            repo_full_name=repo_full_name,
            github_token=github_token,
            branch_name=prepared_branch.branch_name,
            base_branch=prepared_branch.base_branch,
            title=pr_title,
            body=pr_description,
        )

        pr_metadata = dict(scan.get("pr_metadata", {}) or {})
        pr_metadata.update({
            "fix_branch": prepared_branch.branch_name,
            "base_branch": prepared_branch.base_branch,
            "patch_s3_key": prepared_branch.patch_s3_key,
            "repo_full_name": repo_full_name,
            "commit_sha": prepared_branch.commit_sha,
            "modified_files_count": prepared_branch.modified_files_count,
        })
        ...
        logger.info(
            ...
            already_existed=pull_request.already_existed,
        )
        ...
        logger.warning("PR creation failed", scan_id=scan_id, error=str(exc))
        return CreatePRResponse(
            success=False,
            error=str(exc),
        )
