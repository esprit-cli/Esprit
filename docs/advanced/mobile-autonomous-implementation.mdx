...
- Keep v1 CLI-only.
- Treat MobSF sidecar as v2+, so primary scan container stays lean and independently testable.
...
- `mobile_action(action="extract_endpoints", app_path)`
  - Returns normalized endpoint candidates with confidence scores and provenance.
...
2. Score endpoint candidates by confidence/provenance:
   - observed in proxy traffic > decoded literal > inferred/constructed.
3. Enforce in-scope host filtering before auto-testing.
4. Convert qualified candidates to prioritized API hypotheses.
5. Use existing proxy/terminal/python tools for replay/fuzz/validation.
6. Report via existing `create_vulnerability_report`.
